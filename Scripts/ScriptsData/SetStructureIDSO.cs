...
#if UNITY_EDITOR
        [CustomEditor(typeof(SetStructureIDSO))]
        class SetStructureIDSOHelperEditor : Editor
        {
            bool showPosition = true;
            public override void OnInspectorGUI()
            {
                SetStructureIDSO targetObject = (SetStructureIDSO)target;

                EditorGUILayout.LabelField("  ", "Лист ID");

                EditorGUILayout.Space();
                showPosition = EditorGUILayout.Foldout(showPosition, "Список ID");
                if (showPosition)
                    for (int i = 0; i < targetObject._listStructureID.Count; i++)
                    {
                        foreach (var b in targetObject._listStructureVidsIndex)
                            if (targetObject._listStructureID[i] == targetObject._listStructureID[b])
                                EditorGUILayout.Space();

                        EditorGUILayout.DelayedTextField((i + 1) + ": ", "ID " + targetObject._listStructureID[i]);
                    }
